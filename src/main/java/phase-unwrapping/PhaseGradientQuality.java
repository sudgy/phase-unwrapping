...
class PhaseGradientQuality implements Quality {
    private float[][] M_data;
    private float M_phase;
    public PhaseGradientQuality(float phase)
    {
        M_phase = phase;
    }
            ...
            for (int y = 0; y < result[0].length; ++y) {
                for (int x_plus = -1; x_plus <= 1; ++x_plus) {
                    for (int y_plus = -1; y_plus <= 1; ++y_plus) {
                        ...
                        float difference = M_data[x][y] - M_data[new_x][new_y];
                        if (difference < -M_phase / 2) difference += M_phase;
                        if (difference > M_phase / 2) difference -= M_phase;
                        result[x][y] -= Math.abs(difference);
                    }
                }
            }
        }
